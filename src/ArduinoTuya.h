...
#define TUYA_DEBUG_SERIAL Serial1
...
#ifdef TUYA_DEBUG_SERIAL
 #define DEBUG_PRINT(x)       TUYA_DEBUG_SERIAL.print (x)
 #define DEBUG_PRINTDEC(x)    TUYA_DEBUG_SERIAL.print (x, DEC)
 #define DEBUG_PRINTHEX(x)    TUYA_DEBUG_SERIAL.print (x, HEX)
 #define DEBUG_PRINTLN(x)     TUYA_DEBUG_SERIAL.println (x)
 #define DEBUG_PRINTJSON(x)   serializeJson(doc, TUYA_DEBUG_SERIAL);TUYA_DEBUG_SERIAL.println()
#else
...
class TuyaDevice {
  ...
  String response;  
    ...
    void begin(const char* id, const char* key, const char* host = NULL, uint16_t port = TUYA_PORT_DEFAULT, const char* version = TUYA_VERSION_DEFAULT) {
      _id = String(id);
      _key = String(key);
      _host = host;
      _port = port;
